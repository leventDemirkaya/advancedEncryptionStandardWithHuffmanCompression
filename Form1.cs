...
using System.Security.Cryptography; //AES için kullanılacaktır.
namespace sesVerileriÜzerineVeriGizleme
{
    public partial class Form1 : Form
    {
        ...
        static byte AesSelection;
        static string AesEncoding;
        static string AesDecoding;
        static byte[]anahtar;
        static byte[]vektor;
    #endregion
        public Form1()
        ...
        private void lstbx_SesDosyalari_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            chbx_AesNo.Enabled = true; chbx_AesNo.Visible = true;
            chbx_AesYes.Enabled = true; chbx_AesYes.Visible = true;
            lbl_Cryptology.Visible = true;
        }
        ...
        private String GizlenenMesajiGoster(byte sesGoruntuVideo)
        {
            if (sesGoruntuVideo == 1)
            {
                ...
                if (AesSelection == 1)
                {
                    AesEncoding = AesSifrele(sonuc);
                    shiftKeyOlustur(AesEncoding, 0);
                    AesDecoding = AesSifreKir(AesEncoding);
                    sonuc = AesDecoding;
                }
                else shiftKeyOlustur(sonuc, 0);
                return sonuc;
            }
            else if (sesGoruntuVideo==2)
            {
                ...
                String path = Application.StartupPath + "\\gizlenenResimSinyali.txt";
            ...
            else
            {
                ...
                String path = Application.StartupPath + "\\gizlenenVideoSinyali.txt";
        ...
        private void DegiskenTemizle()
        {
            ...
            huffmanResimVideo = "";
            AesSelection = 0;
            chbx_AesNo.Checked = false; chbx_AesYes.Checked = false;
            AesEncoding = ""; AesDecoding = "";
        }
        ...
        private void bt_rdBtSecimi_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (chbx_AesNo.Checked) AesSelection = 0;
                if (chbx_AesYes.Checked) AesSelection = 1;
                if (chbx_AesNo.Checked && chbx_AesYes.Checked) AesSelection = 0;
        ...
        private void arrayToStego(String path, byte goruntuVideo)
        {
            if (goruntuVideo == 1)
            {
                if (File.Exists(@path))
                {
                    ...
                    {
                        MessageBox.Show($"Resim Dosyasını Oluşturma Sırasında Hata Oluştu : {e.Message}");
                    }
                }
            }
            else if(goruntuVideo==2)
            {
                ...
                if (File.Exists(@path)) // Parametre olarak gelen path'te dosya bulunuyorsa
                {
                    ...
                    {
                        MessageBox.Show($"Video Dosyasını Oluşturma Sırasında Hata Oluştu : {e.Message}");
                    }
                }
            }
        }
        ...
        private void huffmanKodOlustur(List<string>birlestirmeler,int arananKelime)
        {
            ...
            string path = Application.StartupPath + "\\huffmanFrekansKod.txt";
            using(StreamWriter sw = new StreamWriter(path))
            {
        ...
        private void gizlenenMesajToString(byte goruntuVideo,String goruntuVideoAdresi)
        {
            ...
            if(goruntuVideo==1)pathHuffman = Application.StartupPath + "\\Aes_HuffmanKoduResim.txt";
            else pathHuffman = Application.StartupPath + "\\Aes_HuffmanKoduVideo.txt";
            ...
            if (AesSelection == 1)
            {
                AesEncoding = AesSifrele(gizlenecekMesaj);
                shiftKeyOlustur(AesEncoding, 0);
                huffmanToStego(goruntuVideo, AesEncoding, pathHuffman);
            }
            else
            {
        ...
        private void huffmanToStego(byte goruntuVideo,string gizlenecekMesaj, string pathHuffman)
        {
            ...
            if (goruntuVideo==1)
            {
                try
                {
                    path = Application.StartupPath + "\\AesHuffmanResimSinyali.txt";
                    using (StreamWriter sw = new StreamWriter(path))
                    {
                        sw.Write(huffmanResimVideo);
                    }
                    byte[] huffmanResimByte;
                    if (AesSelection == 1)
                    {
                        AesDecoding = AesSifreKir(huffmanResimVideo);
                        huffmanResimByte = Convert.FromBase64String(AesDecoding);
                    }
                    else huffmanResimByte = Convert.FromBase64String(File.ReadAllText(path));
                    using (MemoryStream ms = new MemoryStream(huffmanResimByte))
                    {
                        Image okunanResim = Image.FromStream(ms);
                        okunanResim.Save($"AEShuffmanResim{calistirilmaSayisi}.jpg", System.Drawing.Imaging.ImageFormat.Jpeg);
                    }
            ...
            {
                String hfvideo_ = $@"AesHuffmanStegoVideo{calistirilmaSayisi}.mp4"; // Oluşturulacak yeni video dosyasının adı
                try
                {
                    byte[] hfVideoByte;
                    if (AesSelection == 1)
                    {
                        AesDecoding = AesSifreKir(huffmanResimVideo);
                        hfVideoByte = Convert.FromBase64String(AesDecoding);
                    }
                    else hfVideoByte = Convert.FromBase64String(huffmanResimVideo);
        ...
        private String AesSifrele(String gizlenecekSinyal)
        {
            byte[] sinyal = Encoding.UTF8.GetBytes(gizlenecekSinyal); //Parametre olan gizlenecekSinyal'in string tipindeki hali
            anahtar = new byte[32]; //256 bit anahtar
            vektor = new byte[16]; //128 bit Initialization Vector
            using (var rnd = new RNGCryptoServiceProvider())
            {
                rnd.GetBytes(anahtar);
                rnd.GetBytes(vektor);
            } //kriptografik olarak güvenli rastgele sayılar
            byte[] stegoSinyal; //String'e return edilmeden içinde değerler tutuluyor
            using (Aes aes = Aes.Create())
            {
                aes.Key = anahtar;
                aes.IV = vektor;
                aes.Mode = CipherMode.CBC; //Block Cipher modu, güvenlik sağlıyor. Kendisi ve bir sonraki blok hata olursa etkilenir.
                aes.Padding = PaddingMode.PKCS7; //Eksik byteların sayısı kadar ekleme yapıyor.
                using (MemoryStream ms = new MemoryStream())
                {
                    using (CryptoStream cs = new CryptoStream(ms, aes.CreateEncryptor(), CryptoStreamMode.Write))
                    {
                        cs.Write(sinyal, 0, sinyal.Length); // Belirtilen byte dizisi cs nesnesine yazılıyor.
                        cs.FlushFinalBlock();
                        //Sifreleme işlemi sırasında, verinin boyutu blok boyutunun tam katı değilse, padding uygulanır. FlushFinalBlock, bu padding'in doğru bir şekilde eklenmesini sağlar.
                    }
                    File.WriteAllBytes(Application.StartupPath + "\\kapakSinyal_AesSifreleme.txt", ms.ToArray());
                    stegoSinyal=ms.ToArray();
                    return Convert.ToBase64String(stegoSinyal);
                }
            }
        }
        private String AesSifreKir(String sinyal)
        {
            byte[]stegoSinyal=Convert.FromBase64String(sinyal);
            using (Aes aes = Aes.Create())
            {
                aes.Key = anahtar;
                aes.IV = vektor;
                ...
                using (MemoryStream ms = new MemoryStream())
                {
                    ...
                    {
                        cs.Write(stegoSinyal, 0, stegoSinyal.Length);// Belirtilen byte dizisi cs nesnesine yazılıyor.
                        cs.FlushFinalBlock();
                        //Sifre cozme işlemi sırasında, verinin boyutu blok boyutunun tam katı değilse, padding uygulanır. FlushFinalBlock, bu padding'in doğru bir şekilde eklenmesini sağlar.
                    }
                    File.WriteAllBytes(Application.StartupPath + "\\stegoSinyal_AesSifreKirma.txt", ms.ToArray());
